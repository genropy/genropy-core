...
try:
    ...
    class LocalizedWrapper(object):
        ...
        def __init__(self, data, locale=None, templates=None, formats=None, masks=None,
                     editcols=None, df_templates=None, dtypes=None, localizer=None,
                     urlformatter=None, noneIsBlank=None, emptyMode=None):
            self.data = data
            self.locale = locale
        ...
        def __getitem__(self, k):
                    ...
                    if self.templates and templatename in self.templates:
                        ...
                        if templateNode:
                            ...
                            for v in value.values():
                                result.append(
                                    templateReplace(template, v,
                                                    locale=self.locale,
                                                    formats=self.formats,
                                                    masks=self.masks,
                                                    editcols=self.editcols,
                                                    dtypes=self.dtypes,
            ...
            if mask and '#' in mask:
                caption = self.localizer.translate(
                    caption) if self.localizer else caption.replace('!!', '')
...
def like(s1, s2, wildcard='%'):
    ...
    if s1.startswith('^'):
        s1 = s1[1:].upper()
        s2 = s2.upper()
    if s1 == wildcard or s1 == s2:
        return True
    elif not wildcard in s1:
        return False
    if s1.startswith(wildcard):
        if s1.endswith(wildcard):
            return bool(s1[1:-1] in s2)
        return bool(s2.endswith(s1[1:]))
    if s1.endswith(wildcard):
        return bool(s2.startswith(s1[:-1]))
    return False
...
def templateReplace(myString, symbolDict=None, safeMode=False, noneIsBlank=True, locale=None,
                    formats=None, dtypes=None, masks=None, editcols=None, df_templates=None,
                    localizer=None, urlformatter=None, emptyMode=None, conditionalMode=True):
    ...
    if not '$' in myString or not symbolDict:
        return myString
    ...
    symbolDict = LocalizedWrapper(symbolDict, locale=locale, templates=templateBag,
                                  noneIsBlank=noneIsBlank, formats=formats, dtypes=dtypes,
...
def asDict(myString, itemSep=',', argSep='=', symbols=None):
    ...
    for item in itemList:
        ...
        if value.startswith('(')and value.endswith(')'):
            ...
            if '$' in value:
                value = templateReplace(value, symbols)
            result[key] = value
    return result
    ...
    while start < end:
        ...
        while not foundsep or wch:
            ...
            if gr[1] == "":  # End of string
                if wch:
                    raise ValueError("wrong level: missing :" + wch.pop())
                foundsep = True
                blockLen = res.end()
                ...
                if char:
                    if char in """\'\"""":
                        nextPos = path.index(char, nextPos) + 1
                    elif char == '(':
                        wch.append(')')
                    elif char == '[':
                        wch.append(']')
