...
class FilterList(list):
    ...
    def __contains__old(self, item):
        return len([k for k in self if
                    k == item or k.endswith('*') and item.startswith(k[0:-1])]) > 0
...
def moduleDict(module, proplist):
    ...
    for prop in [x.strip() for x in proplist.split(',')]:
        modulelist = [getattr(module, x) for x in dir(module) if
                      hasattr(getattr(module, x), prop) and getattr(getattr(module, x),
                                                                    '__module__',
...
class GnrException(Exception):
    ...
    def localizedMsg(self, msg, msgargs):
        ...
        # msgargs is use 2 times as we could have msgargs nested(max 1 level)
        return msg % msgargs % msgargs
...
class GnrAddOn(object):
    ...
    def recorderWrite(self):
        ...
        if srcargs:
            args.extend(vlocals[srcargs])
        kwargs = dict([(key, vlocals[key]) for key in srcargname[nargs:]])
        if srckwargs:
            kwargs.update(vlocals[srckwargs])
...
class GnrRemeberableAddOn(GnrAddOn):
    ...
    def __del__(self):
        try:
            ...
            if self._gnr_remembered_as__:
                self._gnr_namedmembers__.pop(self._gnr_remembered_as__)
        except:
            pass
        object.__del__(self)
    ...
    def rememberMe(self, name=None):
        ...
        if name:
            ...
            if old:
                self._gnr_members__[old]._gnr_remembered_as__ = None
...
def classMixin(target_class, source_class, methods=None, only_callables=True,
    ...
    exclude_list = dir(type) + ['__weakref__', '__onmixin__',
                                '__on_class_mixin__', '__py_requires__', 'proxy']
    if exclude:
...
def instanceMixin(obj, source, methods=None, attributes=None, only_callables=True,
    ...
    if isinstance(source, basestring):
        ...
        for clsname in classes:
            ...
            if source:
                instanceMixin(obj, source, methods=methods, only_callables=only_callables,
                              exclude=exclude, prefix=prefix, suffix=suffix, _mixined=_mixined,
                              **kwargs)
        return _mixined
    if source is None:
        return
    ...
    mlist = [k for k in source_dir if
             callable(getattr(source, k)) and not k in dir(type) + ['__weakref__',
                                                                    '__onmixin__',
                                                                    'mixin']]
...
def _calledAync(call=None, call_args=None, call_kwargs=None, cb=None, cb_args=None,
                cb_kwargs=None, status_dict=None):
...
def callAsync(call=None, call_args=None, call_kwargs=None, cb=None, cb_args=None,
              cb_kwargs=None, exit_timeout=60):
