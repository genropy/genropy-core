...
class GnrClassCatalog(object):
    ...
    def isTypedText(self, txt, **kwargs):
        if not '::' in txt:
            return False
        return txt.split('::')[-1].upper() in ['HTML', 'JS', 'RPC', 'JSON', 'NN', 'BAG',
                                               'A', 'T', 'L', 'N', 'I', 'B', 'D', 'H',
                                               'HZ', 'DH', 'DHZ', 'P', 'X']
    ...
    def asTypedText(self, o, quoted=False, translate_cb=None, jsmode=False):
        ...
        if t == 'T':
            ...
            result = "%s::%s" % (self.asText(o, translate_cb=translate_cb,
                                             jsmode=jsmode), self.names[type(o)])
        if quoted:
            result = self.quoted(result)
        return result
    ...
    def standardClasses(self):
        ...
        self.addClass(cls=datetime.datetime, key='DH', aliases=[
                      'DATETIME', 'DT', 'DHZ'], empty=None)
        ...
        self.addClass(cls=Decimal, key='N', aliases=[
                      'NUMERIC', 'DECIMAL'], align='R', empty=Decimal('0'))
