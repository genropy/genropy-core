    ...
    * *attributes*: dictionary that contains node's metadata"""
        ...
        if not self._resolver == None:
            if 'static' in mode:
                ...
                if self._resolver.readOnly:
                    # value is not saved in bag, eventually is cached by resolver or lost
    ...
    def getFormattedValue(self, joiner=None, omitEmpty=True, mode='', **kwargs):
        ...
        if v or not omitEmpty:
            return '%s: %s' % ((self.attr.get('_valuelabel') or self.attr.get('name_long')
                                or self.label.capitalize()), v)
        return ''
    ...
    def setAttr(self, attr=None, trigger=True, _updattr=True, _removeNullAttributes=True,
                **kwargs):
        ...
        if trigger:
            if self._node_subscribers:
                upd_attrs = [k for k in self.attr.keys() if (
                    k not in oldattr.keys() or self.attr[k] != oldattr[k])]
                for subscriber in self._node_subscribers.values():
...
class Bag(GnrObject):
    ...
    def __str__(self, exploredNodes=None, mode='static,weak'):
        ...
        for idx, el in enumerate(self._nodes):
            ...
            if isinstance(value, Bag):
                ...
                if currtype == 'NoneType':
                    currtype = 'None'
                if '.' in currtype:
                    currtype = currtype.split('.')[-1]
    ...
    def diff(self, other):
        ...
        for k, n in enumerate(self._nodes):
            if n != other._nodes[k]:
                result.append('Node %i label %s difference %s' %
                              (k, n.label, n.diff(other._nodes[k])))
        return '\n'.join(result)
        ...
        for node in self.nodes:
            ...
            if k in othernodes:
                ...
                if isinstance(v, Bag) and isinstance(ov, Bag):
                    v = v.merge(ov, upd_values=upd_values, add_values=add_values,
    ...
    def _fromJson(self, json, listJoiner=None):
        ...
        if isinstance(json, list):
            ...
            for n, v in enumerate(json):
                result.setItem('r_%i' % n, self._fromJson(
                    v, listJoiner=listJoiner), _autolist=True)
    ...
    def _onNodeChanged(self, node, pathlist, evt, oldvalue=None, reason=None):
        ...
        if self.parent:
            self.parent._onNodeChanged(
                node, [self.parentNode.label] + pathlist, evt, oldvalue, reason=reason)
    ...
    def _onNodeInserted(self, node, ind, pathlist=None, reason=None):
        ...
        if self.parent:
            self.parent._onNodeInserted(
                node, ind, [self.parentNode.label] + pathlist, reason=reason)
    ...
    def child(self, tag, childname='*_#', childcontent=None, _parentTag=None, **kwargs):
        ...
        if _parentTag:
            ...
            if not actualParentTag in _parentTag:
                raise BagException('%s "%s" cannot be inserted in a %s' %
        ...
        if childname in where and where[childname] != '' and where[childname] is not None:
            if where.getAttr(childname, 'tag') != tag:
                raise BagException('Cannot change %s from %s to %s' %
...
class GeoCoderBag(Bag):
    def setGeocode(self, key, address, language='it'):
        ...
        url = "http://maps.googleapis.com/maps/api/geocode/xml?%s" % six.moves.urllib.parse.urlencode(
            urlparams)
        self._result = Bag()
        answer = Bag(url)
...
class DirectoryResolver(BagResolver):
    ...
    def load(self):
        """TODO"""
        extensions = dict([((ext.split(':') + (ext.split(':'))))[0:2]
                           for ext in self.ext.split(',')]) if self.ext else dict()
        ...
        for fname in directory:
            ...
            if os.path.isdir(fullpath):
                ...
                if self.include or self.exclude:
                    addIt = gnrstring.filter(fname, include=self.include,
                                             exclude=self.exclude, wildcard='*')
            ...
            if addIt:
                ...
                caption = '!!%s %s' % (str(int(m.group(1))), m.group(
                    2).capitalize()) if m else caption.capitalize()
