...
def localize_img(value, locale, format=None, **kwargs):
    ...
    if 'h' in format and 'w' in format:
        ...
        if cropper_zoom:
            cropper_style = """-webkit-transform:scale(%f);-webkit-transform-origin:0px 0px;-moz-transform:scale(%f);-moz-transform-origin:0px 0px;""" % (
                cropper_zoom, cropper_zoom)
...
def getMonthNames(locale=None):
    ...
    d.update([(v.lower(), k) for k, v in dates.get_month_names(
        width='abbreviated', locale=locale).items()])
...
def getQuarterNames(locale=None):
    ...
    d.update([(v.lower(), k) for k, v in dates.get_quarter_names(
        width='abbreviated', locale=locale).items()])
    return d
...
DATEKEYWORDS = {
    'en': {'this week': ('this week', 'week'), 'next week': 'next week', 'last week': 'last week',
           'this month': ('this month', 'month'), 'next month': 'next month',
           'last month': 'last month', 'today': 'today', 'yesterday': 'yesterday',
           'tomorrow': 'tomorrow', 'to': ('to', 'and'), 'from': ('from', 'between'),
           'no period': ('no period', 'always')},
    'it': {'this week': ('questa settimana', 'settimana'), 'next week': 'settimana prossima',
           'last week': 'settimana scorsa',
           'this month': ('questo mese', 'mese'), 'next month': 'mese prossimo',
           'last month': 'mese scorso', 'today': 'oggi', 'yesterday': 'ieri',
           'tomorrow': 'domani', 'from': ('da', 'dal', 'dalla'), 'to': ('a', 'al', 'alla', 'e'),
           'no period': ('-', 'senza periodo', 'sempre')}
}
